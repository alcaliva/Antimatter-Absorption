...
#define GRIDWorkingDir  "JULIA_DIR"
#define AnalysisMacro   "AnalysisJulia"
#define AnalysisTask    "AliAnalysisTaskJulia"
#define TTL             20000
...
void LoadAnalysisTask (AliAnalysisManager *mgr)  {
    ...
    //Load Analysis Tasks
    gROOT->LoadMacro("AliAnalysisTaskJulia.cxx+g");
    
    //Input container
    AliAnalysisDataContainer *input = mgr -> GetCommonInputContainer();

    //Analysis Task
    AliAnalysisTaskJulia *task = new AliAnalysisTaskJulia  ("AliAnalysisTaskJulia");
    task -> SelectCollisionCandidates (AliVEvent::kINT7);
    task -> AliAnalysisTaskJulia::SetAnalysisCuts (CentralityMin,CentralityMax,VertexZmin,VertexZmax,NumberOfVertexContributorsMin,MagFieldSign,DCAxyMax,DCAzMax,PtMin,PtMax,EtaMax,NumberOfClustersITSMin,NumberOfClustersTPCMin,NumberOfClustersTRDMin,NumberOfClustersTPCdEdxMin,NumberOfCrossedRowsMin,CrossedRowsOverFindableClsMin,ChiSquarePerNDFMax,ChiSquareTRDMax,NumberOfTRDtrackletsPIDMin,IsTRDrefit,TPCnsigmaDeuteronMax,TOFnsigmaDeuteronMax);
    mgr -> AddTask(task);
